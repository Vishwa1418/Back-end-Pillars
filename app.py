...
def login():
    if request.method == "POST":
        ...
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM accounts WHERE email = %s AND password = %s',(user['username'],user['password']))
...
def signup():
    if request.method == "POST" :
        ...
        cursor.execute('SELECT * FROM accounts WHERE email = %s',(user['username']))
        account = cursor.fetchone()
        if account:
